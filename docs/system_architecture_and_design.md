...
```mermaid
flowchart LR
    A[Data Generation\n(generate_data.py)] --> B[Raw Data Zone\ndata/raw/*.csv]

    B --> C[Ingestion Layer\ningestion.py\nSchema + null audits]
    C --> D[Transformation Layer\ntransform.py\nType casting + KPI derivations + master join]
    D --> E[Load Layer\nload.py\nSQLite tables + SQL views]

    E --> F[(SQLite\ndata/supply_chain.db)]
    D --> G[Processed Zone\ndata/processed/*.csv]

    G --> H[ML Training Layer\nmodels.py\nXGBoost classifier + regressors]
    H --> I[Model Artifacts\nmodels/*.pkl]

    F --> J[BI App\ndashboard/app.py]
    J --> K[Business Users\nOperations, Procurement, Leadership]
